...
const RANKING_DATA = [
  { id: 1, name: 'Miruu', score: 3702, attended: 150, flag: '👑' },
  { id: 2, name: 'Neal Wu', score: 3686, attended: 142, flag: '🇺🇸' },
  { id: 3, name: 'Yawn_Sean', score: 3644, attended: 138, flag: '🇬🇧' },
  { id: 4, name: '小羊肖恩', score: 3611, attended: 107, flag: '🇨🇳' },
  { id: 5, name: '何逊', score: 3599, attended: 146, flag: '🇨🇳' },
  { id: 6, name: 'Joshua Chen', score: 3589, attended: 100, flag: '🇦🇺' },
  { id: 7, name: 'Rohin Garg', score: 3506, attended: 88, flag: '🇮🇳' },
  { id: 8, name: 'SSerxhs', score: 3499, attended: 61, flag: '🌍' },
  { id: 9, name: '小咩肖恩', score: 3490, attended: 50, flag: '🇨🇳' },
  { id: 10, name: 'fmota', score: 3453, attended: 65, flag: '🇧🇷' },
]
...
export function RankingBoard() {
  ...
  return (
    <div className="flex h-full w-full font-sans">
      ...
      {/* === ŚRODKOWY KONTENT === */}
      <div className="flex shrink-0 flex-col justify-center px-4 py-8 lg:px-12">
        <div className="flex flex-col items-center gap-10 lg:flex-row lg:items-start lg:justify-center lg:gap-16">
          ...
          <div className="flex w-full max-w-md flex-col gap-2">
            {rest.map((user) => (
              <div
                key={user.id}
                className="border-foreground/10 bg-background/80 hover:bg-foreground/[0.04] flex items-center gap-3 rounded-xl border p-2 px-3 backdrop-blur-sm transition-colors sm:p-3 sm:px-4"
              >
                <div className="bg-foreground/10 text-foreground/80 flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold sm:h-8 sm:w-8 sm:text-sm">
                  {user.id}
                </div>
